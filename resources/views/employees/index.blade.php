            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <a href="{{ route('employees.create') }}">
                        <x-button class="bg-blue-500 hover:bg-blue-600 text-white mb-4">Crear Empleado</x-button>
                    </a>                   
                    ...
                    <x-table>
                        <x-slot name="thead">
                            <tr class="border-b-2 border-gray-200">
                                ...
                                <th class="py-3 px-4 text-start text-sm font-semibold tracking-wider text-gray-700 uppercase">Teléfono</th>
                                <th class="py-3 px-4 text-start text-sm font-semibold tracking-wider text-gray-700 uppercase">Empresa</th>
                        ...
                        <x-slot name="tbody">
                            @foreach ($employees as $index => $employee)
                                <tr class="hover:bg-blue-100">
                                    <td class="py-3 px-4">{{ $employees->firstItem() + $loop->index }}</td>
                                    <td class="py-3 px-4">{{ $employee->first_name }} {{ $employee->last_name }}</td>
                                    <td class="py-3 px-4">{{ $employee->email ?? 'No Email' }}</td>
                                    ...
                                    <td class="py-3 px-4">
                                        <div class="flex gap-2">
                                            ...
                                            <a href="#" onclick="confirmDelete('{{ route('employees.destroy', $employee) }}')">
